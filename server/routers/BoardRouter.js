...
const Board = (db) => {
  router.get("/api/getPostList", async (req, res) => {
    const boardType =
      req.query.type && req.query.type !== "all" ? req.query.type : "";
    ...
    try {
      ...
      if (postList.length > 0) {
        ...
        return res
          .status(404)
          .json({ message: "해당 타입의 게시글이 존재하지 않습니다." });
    } catch (e) {
  ...
  router.get("/api/getUserPostList", async (req, res) => {
    ...
    try {
      ...
      if (postList) {
        ...
        return res
          .status(404)
          .json({ message: "해당 유저의 게시글이 존재하지 않습니다." });
      }
    } catch (e) {
      return res.status(500).json({ message: "Server Error!!" });
    }
  ...
  router.delete("/api/deletePosts", async (req, res) => {
    try {
      ...
      for (const post of posts) {
        ...
        if (find) {
          await db.collection("Post").deleteOne({ _id: new ObjectId(post.pk) });
        } else
          return res
            .status(404)
  ...
  router.post("/api/posting", async (req, res) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "게시글 올리기 완료", _id: newPost.insertedId });
    } catch (e) {
      return res.status(500).json({ message: "Server Error!!" });
    }
  });
